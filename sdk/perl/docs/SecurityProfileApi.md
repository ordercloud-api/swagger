# WWW::SwaggerClient::SecurityProfileApi

## Load the API package
```perl
use WWW::SwaggerClient::Object::SecurityProfileApi;
```
...
# **get**
> object get(security_profile_id => $security_profile_id)
...
my $api_instance = WWW::SwaggerClient::SecurityProfileApi->new();
my $security_profile_id = 'security_profile_id_example'; # string | ID of the security profile.

eval { 
    my $result = $api_instance->get(security_profile_id => $security_profile_id);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling SecurityProfileApi->get: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling SecurityProfileApi->list: $@\n";
}
```
