...
class SecurityProfileApi (implicit val swagger: Swagger) extends ScalatraServlet
    with FileUploadSupport
    with JacksonJsonSupport
    with SwaggerSupport {
  protected implicit val jsonFormats: Formats = DefaultFormats

  protected val applicationDescription: String = "SecurityProfileApi"
  override protected val applicationName: Option[String] = Some("/v1/SecurityProfile")
  ...
  val getOperation = (apiOperation[Object]("get")
      summary ""
      parameters(pathParam[String]("securityProfileID").description(""))
  )

  get("/SecurityProfiles/{securityProfileID}",operation(getOperation)) {
    
    
      val securityProfileID = params.getOrElse("securityProfileID", halt(400))
    
    println("securityProfileID: " + securityProfileID)
  }
  ...
  get("/SecurityProfiles",operation(listOperation)) {
